...
MAINL:  li t2,0xFF200000	# carrega o endereço de controle do KDMMIO ("teclado")
	...
	andi t0,t0,0x0001	# mascara todos os bits de t0 com exceçao do bit menos significativo
  	...
  	beq s6,t1,DELETE	# se s6 for igual a 97 (valor hex de "a"), vá para DELETE
  	
  	li t1,119		# carrega 119 (valor hex de "w") para t1
  	beq s6,t1,DELETE	# se s6 for igual a 119 (valor hex de "w"), vá para DELETE
  	
  	li t1,115		# carrega 115 (valor hex de "s") para t1
  	beq s6,t1,DELETE	# se s6 for igual a 115 (valor hex de "s"), vá para DELETE
  	
  	li t1,100  		# carrega 100 (valor hex de "d") para t1
	beq s6,t1,DELETE	# se s6 for igual a 100 (valor hex de "d"), vá para DELETE
...
VERIFY: li t1,97		# carrega 97 (valor hex de "a") para t1
  	beq s6,t1,MOVLFT	# se s6 for igual a 97 (valor hex de "a"), vá para MOVLFT
  	
  	li t1,119		# carrega 119 (valor hex de "w") para t1
  	beq s6,t1,MOVUP	        # se s6 for igual a 119 (valor hex de "w"), vá para MOVUP
  	
  	li t1,115		# carrega 115 (valor hex de "s") para t1
  	beq s6,t1,MOVDWN	# se s6 for igual a 115 (valor hex de "s"), vá para MOVDWN
  	
  	li t1,100  		# carrega 100 (valor hex de "d") para t1
	beq s6,t1,MOVRGHT	# se s6 for igual a 100 (valor hex de "d"), vá para MOVRGHT
